...
var geohubRepoDescription = "Use the following query parameters to output GeoServices info from a GeoJSON Source:" + 
	"<ul>" + 
	...
	"If this is omitted, the first geoJSON Geometry will define the type used to filter on. Note, this is ignored if f=geojson</li>" + 
	"</ul>" +
	"You can also specify the URL as .../geohub/rest/services/repo/<b>githubUsername</b>/<b>repoName</b>/<b>filePath</b>/FeatureService/0<br/>" +
	"If you use the URL approach, encode path separators in the <b>filePath</b> portion of the URL";

var geohubGistDescription = "Use the following query parameters to output GeoServices info from a GeoJSON Source:" + 
	"<ul>" + 
	...
	"If the requested format is geojson (f=geojson) then you may use * for the layerId to return all encountered geoJSON in the gist." + 
	"<br/>You can also specify the URL as .../geohub/rest/services/gist/<b>gistId</b>/FeatureService/<b>gistFileIndex</b>" + 
	"<br/>For example, to find the 2nd file in gist 6178185, use this URL:<br/><ul><li>.../geohub/rest/services/gist/6178185/FeatureService/1</li></ul>";
...
GeoHubProvider = function(app, agsoutput) {
    ...
    this._geohub_repoLayerQueryUrl = this._geohub_defaultLayerQueryUrl.
    	replace(":serviceId", ":serviceId/:githubUser/:repoName/:filePath");
    this._geohub_gistLayerQueryUrl = this._geohub_defaultLayerQueryUrl.
    	replace(":serviceId", ":serviceId/:gistId");
...
Object.defineProperties(GeoHubProvider.prototype, {
    ...
    featureServiceDetails: {
        value: function(detailsTemplate, serviceId) {
            if (serviceId === "repo") {
                ...
                detailsTemplate.serviceDescription = "Whoops - unrecognized GeoHub type. Run Away!";
    ...
    featuresForQuery: {
        value: function(serviceId, layerId, query, callback) {
            if (serviceId === "repo") {
                if (!(query.rawParams.hasOwnProperty("githubUser") && query.rawParams.hasOwnProperty("repoName") && query.rawParams.hasOwnProperty("filePath"))) {
                ...
                Geohub.gist({id:gistId}, function(err, geoJSONData) {
                    if (err) {
                        console.log(err);
                        callback([], err);
                    } else {
